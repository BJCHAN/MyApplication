 ...
 * Date：2014-9-26
 */
package com.chans.countDownTimerView;
...
import com.chans.myapplication.R;
...
public class RushBuyCountDownTimerView extends LinearLayout {
    ...
    private OverTimeInterface overTime;
    ...
    public void setTime(int day, int hour, int min, int sec) {
        //小时24
        if (day > 366 || day < 0 || hour >= 24 || min >= 60 || sec >= 60 || hour < 0 || min < 0
                || sec < 0) {
    ...
    private boolean isCarry4Day(TextView tv) {
        int time = Integer.valueOf(tv.getText().toString());
        time = time - 1;
        if (time < 0) {
            time = 0;
    ...
    public void setOverTime(OverTimeInterface overTime) {
